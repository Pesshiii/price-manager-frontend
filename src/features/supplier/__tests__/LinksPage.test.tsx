...
function baseHandlers(links = [LINK]) {
  return [
    http.get('/api/suppliers/links/', () => HttpResponse.json(links)),
...
describe('LinksPage', () => {
  ...
  it('renders link rows with supplier_sku and product_name', async () => {
    renderPage();
  ...
  it('delete button calls DELETE /suppliers/links/1/ and removes the row', async () => {
    const user = userEvent.setup();
    const confirmSpy = vi.spyOn(window, 'confirm').mockReturnValue(true);

    let deleteCalled = false;
    server.use(
      http.delete('/api/suppliers/links/1/', () => {
        deleteCalled = true;
        return new HttpResponse(null, { status: 204 });
      }),
      http.get('/api/suppliers/links/', () =>
  ...
  it('selecting a product and submitting PATCHes /suppliers/links/1/ and updates the row', async () => {
    let patchBody: unknown;
    const updatedLink = { ...LINK, product: 99, product_name: 'Товар Beta', product_sku: 'INT-099' };

    server.use(
      http.patch('/api/suppliers/links/1/', async ({ request }) => {
      ...
      http.get('/api/suppliers/links/', () => HttpResponse.json([updatedLink])),
    );
    ...
    await waitFor(() => {
      expect(patchBody).toMatchObject({ product: 99 });
    });
  ...
  it('supplier + SKU filters together send both params in the API request', async () => {
    let capturedUrl: string | null = null;
    server.use(
      http.get('/api/suppliers/links/', ({ request }) => {
        capturedUrl = request.url;
    ...
    await waitFor(() => {
      ...
      expect(capturedUrl).toContain('sku=SUPP');
    }, { timeout: 1500 });
  });

  // ── Slice 3: SKU input writes ?sku= to URL ───────────────────────────────

  it('typing in the SKU input writes ?sku= to the URL', async () => {
    let capturedUrl: string | null = null;
    server.use(
      http.get('/api/suppliers/links/', ({ request }) => {
        capturedUrl = request.url;
    ...
    await waitFor(() => {
      expect(capturedUrl).toContain('sku=SUPP-001');
    }, { timeout: 1500 });
  });
